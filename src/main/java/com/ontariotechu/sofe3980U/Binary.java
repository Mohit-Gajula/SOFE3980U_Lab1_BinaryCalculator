...
public class Binary
{
	...
	*/
    ...
    public static Binary or(Binary num1, Binary num2)
    {
       // the index of the first digit of each number
		...
		while(ind1>=0 ||  ind2>=0) // loop until all digits are processed
		{
            ...
            int digit2;
            if(ind2>=0 && num2.number.charAt(ind2)=='1'){				
				digit2 = 1;
			}
            else{
                digit2 = 0;
    ...
    {
        Binary sum = new Binary("0"); // starting with 0
		...
		for (int i = ind2; i>=0; i--)
            {
                ...
                {
                    String shiftedNum = num1.number;
                    int shift = ind2 - i;
                    for (int k = 0; k < shift; k++)
                    {
                        shiftedNum = shiftedNum +"0";
                    }

                    Binary toAdd = new Binary(shiftedNum);
                    sum = Binary.add(sum, toAdd);
                }
            } 
            return sum;
    }
